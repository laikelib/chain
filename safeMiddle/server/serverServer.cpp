...
HRET CServerServer::recvRequest () throw (HCException) {
	...
	LOG_NORMAL("Read MQ for Control{type:[%d], nid:[%d], bid:[%d], len:[%d]}",
		m_ctl.GetType(), m_ctl.GetNid(), m_ctl.GetBid(), m_ctl.GetLen());
...
HRET CServerServer::sendResponse () throw (HCException) {
    ...
    LOG_NORMAL("return send control: {type: [%d], nid: [%d], bid: [%d], len: [%d]}", m_ctl.GetType(), m_ctl.GetNid(), m_ctl.GetBid(), m_ctl.GetLen());
