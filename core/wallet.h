...
public:
    ...
    bool InnerTransfer (CEntry& en);

    bool Transfer (CEntry& en, HCSTRR strPublic, HCSTRR strPrivate);

    void GetTxs (const CLKAddress& addr, std::vector<CEntry>& entrys) const;

    HN64 GetAccountBal (const CLKAddress& addr) const;

    CKey GetKey (const CLKAddress& addr) const;
    ...
    void MakeCoinBase (CEntry& entry, HCSTRR strAddr) const;
