...
HSTR CMainChain::GetBlockTemplate () {
    ...
    LOG(INFO) << "CMainChain::GetBlockTemplate() Begin...";

    shared_ptr<CBlock> pbt (GetNewBlock());
...
HSTR CMainChain::HeightBlock (HCSTRR str) {
    ...
    sp->ReadFromDisk (pindex);

    return sp->ToJsonString();
...
CBlock* CMainChain::GetNewBlock() {
    ...
    CEntry baseEntry;
    m_pWallet->MakeCoinBase(baseEntry);
...
void CMainChain::GetWillBlockEntrys(std::vector<CEntry>& entrys) {
    ...
    UNENTRYLIST::iterator it = m_listUnentry.begin();

    // TODO: not 100, just size.
    while (it != m_listUnentry.end() && ncount < 100) {

	const CEntry& cr = *it;

	it = m_listUnentry.erase(it);
